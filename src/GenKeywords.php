 ...
 */
namespace Cityware\Seo\GenKeywords;
/**
...
class GenKeywords {
    ...
    private $encoding;
    private $lang;
    private $ignore; // array; languages to ignore
    // generated keywords
    private $keywords;
    ...
    public function __construct($params) {
        ...
        if (!isset($params['lang'])) {
            ...
            $this->lang = $params['lang']; // case sensitive
        }
        ...
        if (!isset($params['encoding'])) {
            ...
            $this->encoding = strtoupper($params['encoding']); // case insensitive
        }
        ...
        if ($this->ignore !== false && in_array($this->lang, $this->ignore)) {
            ...
            $this->contents = $this->processText($params['content']);
        }
    ...
    public function getKeywords() {
        ...
        $onew_arr = $this->parseWords();
        $twow_arr = $this->parse2words();
        $thrw_arr = $this->parse3words();
    ...
    private function parseWords() {
        ...
        $str = $this->stripPunctuations(implode(' ', $this->contents));
        ...
        return $this->occureFilter($k, $this->wordOccuredMin);
    }

    // 2-word phrases
    private function parse2words() {
        ...
        return $this->occureFilter($y, $this->phrase2WordLengthMinOccur);
    }

    // 3-word phrases
    private function parse3words() {
        ...
        // count the words; this is the real magic!
        $y = array_count_values($y);
        return $this->occureFilter($y, $this->phrase3WordLengthMinOccur);
    }

    //------------------------------------------------------------------
    private function occureFilter($array, $min) {
        $cnt = 0;
    ...
    //------------------------------------------------------------------
    public function removeDuplicateKeywords($str) {
        if ($str == '') {
            return $str;
        }
